...
export class BatteryModule extends BaseModule {
  ...
  async render(context: ModuleContext): Promise<ModuleResult | null> {
    ...
    let symbol = '🔋'
    if (isCharging) symbol = '🔌'
    else if (isLow) symbol = '🪫'
...
export class CmdDurationModule extends BaseModule {
  ...
  async render(context: ModuleContext): Promise<ModuleResult | null> {
    const durationMs = parseInt(context.environment.CMD_DURATION_MS || context.environment.STARSHIP_DURATION || '0', 10)
    
    if (durationMs < 2000) return null // Only show for commands > 2s
...
export class NixShellModule extends BaseModule {
  ...
  async render(context: ModuleContext): Promise<ModuleResult | null> {
    ...
    if (!inNixShell && !packages) return null
