...
import type { ModuleContext, ModuleResult, GitInfo } from '../types'
...
export class GitBranchModule extends BaseModule {
  ...
  async render(context: ModuleContext): Promise<ModuleResult | null> {
    ...
    if (!gitInfo?.isRepo || !gitInfo.branch) return null
...
export class GitStateModule extends BaseModule {
  ...
  async render(context: ModuleContext): Promise<ModuleResult | null> {
    ...
    const stateMap: Record<string, { symbol: string, color: string }> = {
      'REBASE': { symbol: 'REBASING', color: '#f59e0b' },
      'MERGE': { symbol: 'MERGING', color: '#ef4444' },
      'CHERRY_PICK': { symbol: 'CHERRY-PICKING', color: '#ec4899' },
      'REVERT': { symbol: 'REVERTING', color: '#8b5cf6' },
      'BISECT': { symbol: 'BISECTING', color: '#06b6d4' },
    }
    
    const stateInfo = stateMap[state] || { symbol: state, color: '#6b7280' }
    return this.formatResult(`(${stateInfo.symbol})`, { color: stateInfo.color })
  }
  ...
  private getGitState(cwd: string): string | null {
    ...
    if (existsSync(join(gitDir, 'REBASE_HEAD'))) return 'REBASE'
  ...
  async render(context: ModuleContext): Promise<ModuleResult | null> {
    try {
      ...
      for (const line of lines) {
        ...
        if (addedStr !== '-') added += parseInt(addedStr, 10) || 0
        if (deletedStr !== '-') deleted += parseInt(deletedStr, 10) || 0
      }
